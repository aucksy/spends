...
| 89 | **AI currency conversion (BYOK, off by default)** | *Tier 2.* A bank alert in another currency is converted on the way in and **shows its working**: `RM 100.00 → ₹1,890.00 · 1 MYR = ₹18.90`, on the review card, in the editor and on the saved transaction. Your own key (Anthropic / OpenAI / Groq / Google Gemini), encrypted on-device, never in a backup, never shown back. Only a rate question is sent. Market it as *"shows you the rate it used"*, and say plainly that the rate is an estimate. |
