...
interface ExpenseDao {
    ...
    /** Apply a recurring rule's new value fields (incl. the paid-with instrument, #6) to its already-
     *  generated (non-trashed) rows (#5, edit-all). */
    @Query("UPDATE expenses SET amountMinor = :amount, merchantRaw = :merchant, note = :note, paymentMethodId = :paymentMethodId, updatedAt = :ts WHERE recurringRuleId = :ruleId AND deletedAt IS NULL")
    suspend fun updateRecurringExpenses(ruleId: Long, amount: Long, merchant: String?, note: String?, paymentMethodId: Long?, ts: Long)
