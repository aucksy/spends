...
class CategoryDrillDownKindTest {
    ...
    /**
     * ONE store for the whole test method.
     *
     * [PeriodSelectionStore] builds its DataStore on a FIXED file path, and says in its own comment that
     * this is only safe because it is a `@Singleton` — "exactly one live DataStore instance per file,
     * which DataStore requires". Constructing a second one inside a single test threw
     * `IllegalStateException` from OkioStorage and took three tests down with it. Sharing one is harmless:
     * each ViewModel copies the selection into its own `MutableStateFlow` on construction, so `setPeriod`
     * on one can never reach the other.
     */
    private lateinit var periodStore: PeriodSelectionStore

    @OptIn(ExperimentalCoroutinesApi::class)
    @Before fun setUp() {
        // `viewModelScope` posts to `Dispatchers.Main`, which under a plain unit test has no
        // implementation — the ViewModel's `stateIn` flow then never emits and a collector waits forever.
     ...
     *
     * JUnit does not order sibling `@After` methods, so a separate `resetMain()` and `close()` could run
     * either way round — and when the database teardown lost that race it ran against a dispatcher that
     * had already been torn down. Closing first and restoring the dispatcher second is the only correct
     * order, so it is written down once rather than left to chance.
     */
    @OptIn(ExperimentalCoroutinesApi::class)
    @After fun tearDown() {
    ...
    private fun seed() {
        ...
        db = Room.inMemoryDatabaseBuilder(context, SpendsDatabase::class.java)
            .allowMainThreadQueries()
            .build()
        runBlocking {
            categoryId = db.categoryDao().insert(
                CategoryEntity(
                    name = "Business",
                    iconKey = "work",
                    colorHex = "#3B82F6",
            ...
            fun input(minor: Long, kind: TxnKind, who: String) = TransactionInput(
                ...
                merchantRaw = who,
                note = null,
                allocations = listOf(AllocationInput(categoryId, minor)),
            )
    ...
    private fun viewModelFor(kind: String?): CategoryTransactionsViewModel {
        ...
        return CategoryTransactionsViewModel(
            ...
            periodSelectionStore = periodStore,
    ...
    @Test fun the_two_sides_never_add_up_to_the_old_mixed_total() = runTest {
        ...
        // The bug's signature was one screen showing the sum of both. Each side must be strictly less.
    ...
    @Test fun the_monthly_average_follows_the_lens_too() = runTest {
        ...
        val income = stateFor(TxnKind.INCOME.name)
        val expense = stateFor(TxnKind.EXPENSE.name)
